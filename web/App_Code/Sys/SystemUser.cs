...
public class SystemUser
{
    ...
    public static SystemUser Login(string username, string password)
    {
        ...
        {
            string sqlStr = "select UserID YH_ID, UserName YH_DLM,UserName YH_XM from [dbo].[User] where UserName=@UserName and UserPassword=@UserPassword and UserLeiXing = 'WEB'";
    ...
    {
        string sqlStr = "select UserID YH_ID, UserName YH_DLM,UserName YH_XM from [dbo].[User] where  UserID = @yh_id and UserLeiXing = 'WEB'";
        SqlCommand cmd = new SqlCommand(sqlStr);
        cmd.Parameters.AddWithValue("@yh_id", userid);
        using (DBConnection dbc = new DBConnection())
        {
