        ...
        function GetData(id, username) {
            ...
            ACS('CZCLZ.Model.GetDetailsByViewByCHB', function (retVal) {
                if (retVal) {
                    ...
                    $(".fc").html(retVal.fc + "单");
                    var menu_html = "";
                    menu_html += "<li><div data-companymodelchildid='' data-company=''>全部</div></li>";
                    for (var i = 0; i < retVal.dt_child.length; i++) {
                        menu_html += "<li><div data-companymodelchildid='" + retVal.dt_child[i]["CompanyModelChildId"] + "' data-company='" + retVal.dt_child[i]["Company"] + "'>" + retVal.dt_child[i]["ShowCompany"] + "</div></li>";
                    }
                    $(".menu").html(menu_html);

                    $(".menu li").click(function () {
                        user = $(this).children().data("companymodelchildid");
                        company = $(this).children().data("company");
                        GetData(id, $(this).children().data("companymodelchildid"), company);
            ...
            }, CS.onError, id, username, company, UserDenno, startTime, endTime);
        }
    </script>
    <script type="text/javascript">
        //初始化地图对象，加载地图
